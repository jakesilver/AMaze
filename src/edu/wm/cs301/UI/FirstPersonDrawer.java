...
public class FirstPersonDrawer  {
	...
	public void redrawPlay(int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		GlobalItems.wrapper.setColor(Color.BLACK);
		GlobalItems.wrapper.WrapperFillRect(0, 0, view_width, view_height/2);
		// draw dark gray background on upper half
		//gc.setColor(Color.darkGray);
		
		GlobalItems.wrapper.setColor(Color.DKGRAY);
		GlobalItems.wrapper.WrapperFillRect(0, view_height/2, view_width, view_height/2);
	 ...
	 * @param nn
	 */
	public void traverse_node(BSPNode nn) {
	...
	private void drawrect(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		GlobalItems.wrapper.setColor(Color.rgb(56,142,142));
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			if (!rset.intersect(x1i, x2))
				break;
			int x2i = GlobalItems.wrapper.getPY(x1i, x2);
			x1i = GlobalItems.wrapper.getPX(x1i, x2);
			...
			GlobalItems.wrapper.fillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
