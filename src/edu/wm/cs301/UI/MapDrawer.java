...
public class MapDrawer {
	...
	public void draw_map(int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		GlobalItems.wrapper.setColor(Color.WHITE);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				GlobalItems.wrapper.setColor(seencells.hasWallOnTop(x, y) ? Color.WHITE : Color.YELLOW);
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					GlobalItems.wrapper.drawLine(nx1, ny1, nx2, ny1);
				...
				GlobalItems.wrapper.setColor(seencells.hasWallOnLeft(x, y) ? Color.WHITE : Color.YELLOW);
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					GlobalItems.wrapper.drawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
	...
	public void draw_currentlocation(int view_dx, int view_dy) {
		GlobalItems.wrapper.setColor(Color.RED);
		...
		GlobalItems.wrapper.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		GlobalItems.wrapper.drawLine(ctrx, ctry, aptx, apty);
		GlobalItems.wrapper.drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		GlobalItems.wrapper.drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	...
	public void draw_solution(int offx, int offy, int px, int py) {
		...
		GlobalItems.wrapper.setColor(Color.MAGENTA);
		...
		while (d > 1) {
			...
			// do the graphics
			GlobalItems.wrapper.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
